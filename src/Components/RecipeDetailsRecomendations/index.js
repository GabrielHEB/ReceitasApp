  ...
  useEffect(() => {
    if (type === 'drink') {
      const getMealsRecomendations = async () => {
        ...
        // Verifique se meals é um array antes de aplicar o filtro
        const firstSixMeals = Array.isArray(meals)
          ? meals.filter((recipe, index) => index < SIX)
          : [];
    ...
    if (type === 'food') {
      const getDrinkRecomendations = async () => {
        ...
        // Verifique se drinks é um array antes de aplicar o filtro
        const firstSixDrinks = Array.isArray(drinks)
          ? drinks.filter((recipe, index) => index < SIX)
          : [];
